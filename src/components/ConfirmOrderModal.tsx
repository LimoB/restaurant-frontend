...
const ConfirmModal = ({
  ...
  try {
    ...
    const cartItems = cart.map((item) => ({
      ...
      price: item.price.toFixed(2), // Ensure price is a string
      comment: item.comment ?? "",  // Optional comment per item
    }));
    ...
    const orderPayload: OrderInput = {
      ...
      cart: cartItems, // 🔁 updated from `order_items` to `cart`
    };
          ...
          <>
            ...
            {!isLoggedIn && !cartIsEmpty && (
              <p className="mt-4 text-sm text-red-600 text-center">
                Please{" "}
                <Link to="/login" className="underline text-blue-600 hover:text-blue-700 font-medium">
                  log in
                </Link>{" "}
